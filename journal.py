...
"""
journal.py
start_new_session(user="", Xscale=0, Yscale=0) or continue_existing_session(session_id_to_continue=-1):
must be called before every logging session
for logging value by value, use write_value(value=0),for logging a whole set of values into one session at one time, 
use write_record(value_set=[])
Note: session_id's are unique within one instance of an Activity and are not re-assigned even if one or more logging sessions are deleted
"""
...
class JournalInteraction():
    ...
    def start_new_session(self, user="", Xscale=0, Yscale=0, logginginterval_status=' ' ):
        ...
        self.append_existing=False
        self.making_row=False
        print "$$journal.py: a new session has been started and the session_id is", self.session_id
        return self.session_id

    def continue_existing_session(self, session_id_to_continue=-1):
        """This needs to be called before attempting to continue any logging session"""
    ...
    def stop_session(self):
		...
		writer1.writerow( [ datum ] )
            ...
            self.making_row = False
            self.append_existing = False
            #print "$$journal.py: session stopped and the csv writer was", writer1
	    try:
                self.jobject = datastore.create()
                try:
                    self.jobject.metadata['title'] = 'Measure Activity log, ' + str(self.logginginterval_status)
                    self.jobject.metadata['keep'] = '0'
                    self.jobject.metadata['buddies'] = ''
                    self.jobject.metadata['preview'] = ''
                    self.jobject.metadata['icon-color'] = profile.get_color().to_string()
    ...
    def take_screenshot(self, waveform_id = 1):
        ...
        ###      Ideally I should be implementing the write_file and read_file methods        
        ...
        try:
            ...
            try:
                jobject.metadata['title'] = _('Waveform ' + str(waveform_id) + ' ,from Measure Activity')
